...
module.exports = {
  async getAll() {
    ...
    const rows = await dbWrapper.all('SELECT * FROM churches ORDER BY id');
    return rows;
  },
  async getById(id) {
    await dbWrapper.initialize();
  ...
  async create(data) {
    ...
    const result = await dbWrapper.insert(
      `INSERT INTO churches (name, senior_pastor, pastor, assistant_pastor, senior_pastor_avatar, pastor_avatar, assistant_pastor_avatar, address, city, state, zip, contact_email, contact_phone, website, logo_url, banner_url, description)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, senior_pastor, pastor, assistant_pastor, senior_pastor_avatar, pastor_avatar, assistant_pastor_avatar, address, city, state, zip, contact_email, contact_phone, website, logo_url, banner_url, description]
    );
    
    // Get the inserted record
    const inserted = await dbWrapper.get('SELECT * FROM churches WHERE id = ?', [result.lastID]);
    return inserted;
  },
  ...
  async remove(id) {
    ...
    const assignedUsers = await dbWrapper.all(
      'SELECT user_id FROM church_admin_assignments WHERE church_id = ?',
      [id]
    );
    
    // Remove all admin assignments for this church
    await dbWrapper.run('DELETE FROM church_admin_assignments WHERE church_id = ?', [id]);
    ...
    await dbWrapper.run('DELETE FROM churches WHERE id = ?', [id]);
